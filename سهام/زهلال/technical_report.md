...
**آخرین قیمت معاملاتی:** 53,490 ریال | **دامنه روز:** 53,490 تا 53,490 ریال  
**حجم معاملات آخرین روز:** 278,298 برگه سهم (میانگین ۲۰ روزه: 3,414,140)
...
- **سقف ماژور دوره (Swing High):** 53,490 ریال
- **کف ماژور دوره (Swing Low):** 35,002 ریال
- **دامنه نوسان واقعی (ATR 14):** 1,895 ریال

### جدول میانگین‌های متحرک نمایی (Exponential Moving Averages)
| شاخص میانگین | مقدار عددی (ریال) | فاصله با قیمت فعلی | موقعیت تکنیکال |
| :--- | :--- | :--- | :--- |
| **EMA 20** | 48,942 | +9.3% | حمایت پویا |
| **EMA 50** | 46,002 | +16.3% | حمایت میان‌مدت |
| **EMA 100** | 42,988 | +24.4% | حمایت / مقاومت تراز میانی |
| **EMA 200** | 40,399 | +32.4% | مرز روند بلندمدت و استراتژیک |

### وضعیت باندهای بولینگر (Bollinger Bands)
- **باند بالایی (Upper Band):** 52,004 ریال
- **باند میانی (SMA 20):** 48,881 ریال
- **باند پایینی (Lower Band):** 45,758 ریال
- **عرض باند (Bandwidth):** 12.8% (سنجش میزان فشردگی نوسانات)

---

## ۲. تحلیل سیستم معاملاتی ایچیموکو (Ichimoku Kinko Hyo)
- **خط تنکان‌سن (Tenkan-sen 9):** 50,245 ریال
- **خط کیجون‌سن (Kijun-sen 26):** 48,245 ریال
- **ابر کومو اسپن A (Senkou Span A):** 53,490 ریال
- **ابر کومو اسپن B (Senkou Span B):** 53,490 ریال
...
- **شاخص قدرت نسبی (RSI 14):** **73.8** (اشباع خرید (احتمال شناسایی سود یا استراحت موقت روند))
- **مکدی (MACD 12,26,9):** 1616.05 | **خط سیگنال:** 1346.54 | **هیستوگرام:** 269.51
...
مبنای محاسبه ترازهای فیبوناچی اصلاحی بر اساس آخرین موج حرکتی بین کف 35,002 و سقف 53,490 ریال:

| نسبت فیبوناچی | سطح قیمتی (ریال) | فاصله با قیمت فعلی | نقش تکنیکال |
| :--- | :--- | :--- | :--- |
| **0.0** | 53,490 | +0.0% | سقف موج (Pivot High) |
| **0.236** | 49,127 | +8.9% | حمایت / مقاومت اول اصلاحی (23.6%) |
| **0.382** | 46,428 | +15.2% | تراز اصلاحی نرمال (38.2%) |
| **0.5** | 44,246 | +20.9% | تراز میانی تعادلی (50.0%) |
| **0.618** | 42,064 | +27.2% | تراز طلایی و پرتقاضا (61.8%) |
| **0.786** | 38,958 | +37.3% | آخرین سد دفاعی روند (78.6%) |
| **1.0** | 35,002 | +52.8% | کف موج (Pivot Low) |

- **نزدیک‌ترین سطح حمایت معتبر:** **49,127 ریال**
- **نزدیک‌ترین سطح مقاومت معتبر:** **53,490 ریال**
...
- **تحلیل وضعیت فعلی نماد:** آخرین قیمت در سطح **53,490 ریال** در مقایسه با میانگین ۲۰ روزه (48,942 ریال) و میانگین ۵۰ روزه (46,002 ریال) موقعیت **صعودی تثبیت‌شده (قیمت بالاتر از میانگین‌های کوتاه و میان‌مدت قرار دارد)** را ثبت کرده است.
...
- **تحلیل وضعیت فعلی نماد:** مقدار فعلی RSI برابر با **73.8** (اشباع خرید (احتمال شناسایی سود یا استراحت موقت روند)) است و هیستوگرام مکدی در وضعیت **+269.51** نوسان می‌کند.
