# جمع‌بندی تحلیلی و پیشنهاد معاملاتی نماد تلیسه

> **📅 تاریخ و ساعت آخرین بروزرسانی:** 1405/06/15 - 19:11  
> **نویسنده و توسعه دهنده:** [tjb24.ir](https://tjb24.ir)  
**سیگنال و وضعیت نهایی:** **خرید پله‌ای (Accumulate)**  
**امتیاز اجماع توصیه معاملاتی:** **3.9 از ۵ (★★★★☆)** | **🟢 خرید / ورود پله‌ای (Buy)**  
**آخرین قیمت معاملاتی:** 9,470 ریال  
**کیفیت ریسک به ریوارد (R/R):** 🔴 نامناسب (R/R < 1.5)  

> **خلاصه اجرایی و چشم‌انداز:** سهم دارای ارزندگی مناسب و شرایط معاملاتی مساعد است؛ ورود مرحله‌ای و پله‌ای در محدوده مشخص‌شده با رعایت اصول مدیریت ریسک توصیه می‌شود.

---

## 🎯 جدول جامع امتیازدهی سه‌گانه توصیه خرید/فروش (مقیاس ۱ تا ۵)
| رویکرد تحلیلی | امتیاز (۱ تا ۵) | مبنا و منطق محاسبه |
| :--- | :---: | :--- |
| **رویکرد ۱: مدل تجمیع وزنی چندعاملی** | **3.9** | بنیادی (9.5/۱۰ با وزن ۳۵٪) + تکنیکال (9.0/۱۰ با وزن ۳۰٪) + تابلوخوانی (2.5/۱۰ با وزن ۲۵٪) + اخبار (6.0/۱۰ با وزن ۱۰٪) |
| **رویکرد ۲: مدل درخت تصمیم و فیلترهای وتو** | **4.0** | گیت مساعد: ارزندگی بنیادی مطلوب و حمایت خریداران حقیقی |
| **رویکرد ۳: مدل همگرایی افق‌های زمانی و R/R** | **3.7** | افق کوتاه‌مدت (5.8/۱۰) + میان‌مدت (9.2/۱۰) + بلندمدت (8.8/۱۰) با ضریب R/R=1.18 |
| **🌟 امتیاز نهایی اجماع (Composite Score)** | **3.9 از ۵ (★★★★☆)** | **🟢 خرید / ورود پله‌ای (Buy)** |

---

## جدول راهنمای معامله (Actionable Plan)
| پارامتر معاملاتی | مقدار پیشنهادی | درصد بازدهی / ریسک | توضیحات و منطق تحلیلی |
| :--- | :--- | :--- | :--- |
| **محدوده خرید بهینه (Entry Zone)** | **9,281 تا 9,659 ریال** | — | بازه ورود مجاز و منطقی برای ورود پله‌ای |
| **حد سود اول (Target 1)** | **10,360 ریال** | **+9.4%** | مقاومت اول و هدف نوسان‌گیری کوتاه‌مدت |
| **حد سود دوم (Target 2)** | **11,396 ریال** | **+20.3%** | سقف کانال ماژور و مقاومت کلیدی میان‌مدت |
| **حد ضرر پویا (Dynamic Stop Loss)** | **8,713 ریال** | **-8.0%** | سطح شکست حمایت پیوت با ضریب نوسان ATR |
| **نسبت سود به ریسک (R/R)** | **1.18** | — | 🔴 نامناسب (R/R < 1.5) |

---

## راهبرد در ۳ افق زمانی
### ۱. کوتاه‌مدت (۱ تا ۴ هفته)
- **استراتژی اجرایی:** نوسان‌گیری با تکیه بر تابلوی معاملات، ورود پول هوشمند و شکست مقاومت اول
- **تارگت هدف:** 10,360 ریال (بازدهی مورد انتظار: 9.4%)
- **حد ضرر:** 8,713 ریال (حداکثر ریسک: 8.0%)

### ۲. میان‌مدت (۱ تا ۳ ماه)
- **استراتژی اجرایی:** بهره‌گیری از شکست الگوهای ماژور و گزارش‌های فعالیت ماهانه کدال
- **تارگت هدف:** 11,396 ریال (بازدهی مورد انتظار: 20.3%)
- **حد ضرر:** 8,452 ریال (ریسک: 10.8%)

### ۳. بلندمدت (۶ تا ۱۲ ماه)
- **استراتژی اجرایی:** دید بنیادی و سرمایه‌گذاری بر پایه ارزش ذاتی، P/E رو به جلو و سود تقسیمی مجمع (DPS)
- **تارگت هدف:** 14,245 ریال (بازدهی مورد انتظار: 50.4%)
- **حد ضرر:** 8,203 ریال (ریسک: 13.4%)

---

## ماتریس همگرایی تحلیلی (Convergence Matrix)
| رکن تحلیلی | وضعیت و داده‌های کلیدی | سیگنال منتج |
| :--- | :--- | :--- |
| **تحلیل تکنیکال** | شاخص RSI: 58.4 - ATR: 406 ریال | مساعد برای صعود |
| **تابلوخوانی و جریان پول** | سرانه قدرت خریدار به فروشنده: 0.65 | تعادل یا برتری عرضه |
| **تحلیل بنیادی و صورت‌های مالی** | نمره ارزیابی: 9.5/۱۰ - نسبت P/E: 6.5 | ارزندگی بالا و حاشیه سود پایدار |
| **اخبار و رویدادهای کدال** | اطلاعیه‌های ماهانه و سنتیمنت رسانه‌ای | محرک‌های مثبت خبری و رشد درآمد |

---

## مدیریت ریسک و اندازه موقعیت در سبد سرمایه‌گذاری (Risk & Portfolio Sizing)
- **حداکثر سهم پیشنهادی از کل پورتفو:** **10.0%**
- **روش چینش و ورود:** ورود در ۳ پله متوالی با فواصل قیمتی ۲ الی ۳ درصدی برای میانگین‌سازی کنترل‌شده.
- **مدیریت حد ضرر متحرک (Trailing Stop):** پس از دستیابی قیمت به ۵۰٪ فاصله تا تارگت اول، حد ضرر را به نقطه سربه‌سر (قیمت ورود میانگین) منتقل نمایید.
- **قانون حفظ سرمایه:** در هیچ شرایطی اجازه ندهید ریسک این تک‌سهم بیش از ۲ درصد کل ارزش سبد دارایی شما را تحت تأثیر قرار دهد.

---

## شروط ابطال تحلیل
- تثبیت کندل روزانه زیر قیمت 8,713 ریال با حجم معاملات فراتر از میانگین ماهانه.
- خروج سنگین نقدینگی حقیقی و نزول نسبت قدرت خریدار به زیر ۰.۷۵ در دو روز متوالی.
- افت محسوس در درآمد یا نرخ‌های فروش ماهانه در سامانه کدال (بیش از ۱۵ درصد نسبت به دوره قبل).
- شکست قطعی کف سوئینگ ماژور در محدوده 4,242 ریال.

---
*📅 تاریخ و ساعت آخرین بروزرسانی: 1405/06/15 - 19:11*
*نویسنده و توسعه دهنده: [tjb24.ir](https://tjb24.ir)*
*سامانه تحلیل بازار سرمایه: [tjb24.ir](https://tjb24.ir)*

---

---

# 🏛️ گواهی رسمی داوری و ممیزی نهایی نماد تلیسه

- **نماد تحت ممیزی:** `تلیسه`
- **تاریخ صدور گواهی:** `1405/06/15 - 19:11`
- **وضعیت ارزیابی:** **🛡️ تأیید ممیزی داور سامانه (APPROVED)**
- **نمره ممیزی کسب‌شده:** **10.0 از ۱۰.۰** (حدنصاب قبولی: ۸.۵)

---

## 📊 کارنامه ممیزی ارکان پنج‌گانه (5-Pillar Scorecard)

| رکن ممیزی | سقف امتیاز | امتیاز مکتسبه | وضعیت | شرح وضعیت |
| :--- | :---: | :---: | :---: | :--- |
| ۱. اصالت و سلامت فایل‌ها (File Integrity) | ۲.۵ | 2.5 | ✅ تأیید | سلامت بایت‌های جادویی PDF/Excel و عدم وجود فایل‌های HTML خطا |
| ۲. جامعیت و تازگی داده‌های بازار (Data Freshness) | ۲.۵ | 2.5 | ✅ تأیید | جامعیت سابقه معاملات TSETMC و تابلوی عمق بازار |
| ۳. پوشش احساسات شبکه‌های اجتماعی (Social Sentiment) | ۱.۵ | 1.5 | ✅ تأیید | پوشش دیدگاه‌های سهامیاب/رهاورد و محاسبه شاخص سنتیمنت |
| ۴. عمق تحلیلی، نمودارها و راهنمای آموزشی (Analytical Depth) | ۲.۰ | 2.0 | ✅ تأیید | تحلیل بنیادی، تکنیکال، نمودارهای سه‌گانه و راهنمای آموزشی |
| ۵. مدل امتیازدهی سه‌گانه و توصیه استراتژیک (3-Tier Recommendation) | ۱.۵ | 1.5 | ✅ تأیید | مدل امتیازدهی سه‌گانه، حد ضرر پویا و تفکیک افق‌های زمانی |
| **مجموع کل امتیازات** | **۱۰.۰** | **10.0** | **APPROVED** | **دارای صلاحیت و تأیید نهایی** |

---

---

**نهاد داور:** سامانه داوری عالی هوشمند بازار سرمایه (Supreme Judge Agent)

*📅 تاریخ و ساعت آخرین بروزرسانی: 1405/06/15 - 19:11*
*نویسنده و توسعه دهنده: [tjb24.ir](https://tjb24.ir)*
*سامانه تحلیل بازار سرمایه: [tjb24.ir](https://tjb24.ir)*

---

## ⚠️ سلب مسئولیت حقوقی و مالی (Financial & Legal Disclaimer)

> **سلب مسئولیت (فارسی):**  
> کلیه اطلاعات، محاسبات، نمودارها، نسبت‌های مالی و ارزیابی‌های مندرج در این گزارش صرفاً با هدف تحلیل داده، آموزش و پژوهش و توسط **سامانه‌های الگوریتمی و مدل‌های هوش مصنوعی (AI)** بر مبنای اطلاعات گذشته‌نگر و آمار رسمی بازار سرمایه استخراج و تدوین گردیده‌اند. این تحلیل‌ها **به هیچ عنوان توصیه، ترغیب، مشاوره سرمایه‌گذاری یا سیگنالی برای خرید، فروش یا نگهداری هیچ‌یک از دارایی‌ها، سهام یا اوراق بهادار تلقی نمی‌شوند.** تصمیم‌گیری نهایی در خصوص هرگونه معامله در بازار بورس، مدیریت سرمایه، ارزیابی ریسک و مسئولیت کامل سود و زیان منحصراً بر عهده شخص سرمایه‌گذار است.
> 
> **Disclaimer (English):**  
> All information, calculations, charts, financial ratios, and evaluations presented in this report are generated by **algorithmic systems and Artificial Intelligence (AI) models** solely for informational, educational, and analytical purposes based on historical and public market data. **Under no circumstances should this content be considered as financial advice, investment recommendation, endorsement, or a signal to buy, sell, or hold any financial instrument or security.** Investing and trading in financial markets involve substantial risk of loss. Every investor is solely responsible for conducting independent due diligence, assessing all inherent risks, and making their own investment decisions.