...
**آخرین قیمت معاملاتی:** 706 ریال | **دامنه روز:** 678 تا 714 ریال  
**حجم معاملات آخرین روز:** 10,158,114,855 برگه سهم (میانگین ۲۰ روزه: 9,548,749,550)
...
- **دامنه نوسان واقعی (ATR 14):** 25 ریال

### جدول میانگین‌های متحرک نمایی (Exponential Moving Averages)
| شاخص میانگین | مقدار عددی (ریال) | فاصله با قیمت فعلی | موقعیت تکنیکال |
| :--- | :--- | :--- | :--- |
| **EMA 20** | 656 | +7.6% | حمایت پویا |
| **EMA 50** | 606 | +16.5% | حمایت میان‌مدت |
| **EMA 100** | 608 | +16.1% | حمایت / مقاومت تراز میانی |
| **EMA 200** | 826 | -14.5% | مرز روند بلندمدت و استراتژیک |

### وضعیت باندهای بولینگر (Bollinger Bands)
- **باند بالایی (Upper Band):** 755 ریال
- **باند میانی (SMA 20):** 648 ریال
- **باند پایینی (Lower Band):** 541 ریال
- **عرض باند (Bandwidth):** 33.0% (سنجش میزان فشردگی نوسانات)

---

## ۲. تحلیل سیستم معاملاتی ایچیموکو (Ichimoku Kinko Hyo)
- **خط تنکان‌سن (Tenkan-sen 9):** 694 ریال
- **خط کیجون‌سن (Kijun-sen 26):** 626 ریال
- **ابر کومو اسپن A (Senkou Span A):** 533 ریال
...
- **مکدی (MACD 12,26,9):** 40.75 | **خط سیگنال:** 36.57 | **هیستوگرام:** 4.17
...
| :--- | :--- | :--- | :--- |
| **0.0** | 736 | -4.1% | سقف موج (Pivot High) |
| **0.236** | 668 | +5.8% | حمایت / مقاومت اول اصلاحی (23.6%) |
| **0.382** | 625 | +12.9% | تراز اصلاحی نرمال (38.2%) |
| **0.5** | 591 | +19.5% | تراز میانی تعادلی (50.0%) |
| **0.618** | 557 | +26.8% | تراز طلایی و پرتقاضا (61.8%) |
| **0.786** | 508 | +39.0% | آخرین سد دفاعی روند (78.6%) |
| **1.0** | 446 | +58.3% | کف موج (Pivot Low) |
...
- **تحلیل وضعیت فعلی نماد:** آخرین قیمت در سطح **706 ریال** در مقایسه با میانگین ۲۰ روزه (656 ریال) و میانگین ۵۰ روزه (606 ریال) موقعیت **صعودی تثبیت‌شده (قیمت بالاتر از میانگین‌های کوتاه و میان‌مدت قرار دارد)** را ثبت کرده است.
...
- **تحلیل وضعیت فعلی نماد:** مقدار فعلی RSI برابر با **69.3** (تکانه مثبت و برتری نسبی خریداران) است و هیستوگرام مکدی در وضعیت **+4.17** نوسان می‌کند.
